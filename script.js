...
function openBookingModal(){
  ...
  async function submit(){
    ...
    // إرسال الطلب إلى جدول orders في Supabase (مرئي فورًا لكل الأجهزة عبر السحابة)
    const { error } = await supabaseClient
      .from('orders')
      .insert([
        {
    ...
    if (error) {
      ...
      showToast("حدث خطأ أثناء حفظ الطلب في قاعدة البيانات", "err");
...
function renderAdmin(){
  ...
  const tabsHtml = isOwner
    ...
    `
    : `
      <button class="tab active" data-tab="orders">الحجوزات (${state.orders.length})</button>
    `;
  ...
  if (!isOwner && state.adminTab !== "orders") state.adminTab = "orders";

  const body = document.getElementById("adminBody");
  if (state.adminTab === "products" && isOwner) renderProductsTab(body);
...
function renderProductsTab(body){
  ...
  document.getElementById("addBtn").onclick = async () => {
    ...
    if (error) {
      ...
      showToast("تعذر إضافة المنتج لقاعدة البيانات", "err");
...
async function renderAdminsTab(body){
  ...
  document.getElementById("addAdminBtn").onclick = async () => {
    ...
    try {
      ...
      errEl.textContent = e.message?.includes("موجود") ? "اسم المستخدم موجود مسبقًا" : "تعذر إضافة المشرفة";
    } finally {
  ...
  try {
    ...
    document.getElementById("staffList").innerHTML = `<p class="hint center">تعذر تحميل قائمة المشرفات.</p>`;
  }
}
